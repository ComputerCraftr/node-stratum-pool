...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    if (rpcData.founderreward) {
        if (rpcData.founderreward.payee) {
            var payeeReward = rpcData.founderreward.amount;

            reward -= payeeReward;
            rewardToPool -= payeeReward;

            var payeeScript = util.addressToScript(rpcData.founderreward.payee);
            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(payeeReward),
                util.varIntBuffer(payeeScript.length),
                payeeScript
    ...
    if (rpcData.superblock) {
        if (rpcData.superblock.payee) {
            var payeeReward = rpcData.superblock.amount;

            reward -= payeeReward;
            rewardToPool -= payeeReward;

            var payeeScript = util.addressToScript(rpcData.superblock.payee);
            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(payeeReward),
                util.varIntBuffer(payeeScript.length),
                payeeScript
