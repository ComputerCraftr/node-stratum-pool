    ...
    function DetectCoinData(finishedCallback) {
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
            ...
            if (!options.coin.reward) {
                if (isNaN(rpcResults.getmininginfo.difficulty) && 'proof-of-stake' in rpcResults.getmininginfo.difficulty)
            ...
            options.initStats = {
                ...
                difficulty: ((options.coin.reward == 'POS') ? rpcResults.getmininginfo.difficulty["proof-of-work"]*algos[options.coin.algorithm].multiplier : rpcResults.getmininginfo.difficulty*algos[options.coin.algorithm].multiplier),
                networkHashRate: (isNaN(rpcResults.getmininginfo.networkhashps) ? 1000000*rpcResults.getmininginfo.netmhashps : rpcResults.getmininginfo.networkhashps)
            };
